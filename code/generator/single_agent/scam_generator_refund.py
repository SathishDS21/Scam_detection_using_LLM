...
def generate_llama_response():
    ...
    A phone conversation between a scammer (caller) attempting to perform refund scam on the receiver.
...
refund_scam_data = pd.DataFrame(columns=['dialogue', 'type', 'label'])
...
for i in range(num_responses):
    ...
    print(full_response)
    refund_scam_data = refund_scam_data._append({'dialogue': full_response, 'type': 'refund', 'label': 1}, ignore_index=True)
...
refund_scam_data = refund_scam_data.map(format_cell)

# # Save the DataFrame to a CSV file
refund_scam_data.to_csv('../data/refund_scam_responses_30.csv', index=False)