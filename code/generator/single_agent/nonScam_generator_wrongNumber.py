...
def generate_llama_response():
    ...
    A phone conversation between a caller who call a wrong and a receiver.
...
wrong_nonscam_data = pd.DataFrame(columns=['dialogue', 'type', 'label'])
...
for i in range(num_responses):
    ...
    print(full_response)
    wrong_nonscam_data = wrong_nonscam_data._append({'dialogue': full_response, 'type': 'wrong', 'label': 0}, ignore_index=True)
...
wrong_nonscam_data = wrong_nonscam_data.map(format_cell)

# Save the DataFrame to a CSV file
wrong_nonscam_data.to_csv('../data/wrong_nonscam_responses_200.csv', index=False)