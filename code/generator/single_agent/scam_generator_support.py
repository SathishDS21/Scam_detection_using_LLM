...
def generate_llama_response():
    ...
    A phone conversation between a scammer (caller) attempting to perform tech support scam on the receiver. 
...
support_scam_data = pd.DataFrame(columns=['dialogue', 'type', 'label'])
...
for i in range(num_responses):
    ...
    print(full_response)
    support_scam_data = support_scam_data._append({'dialogue': full_response, 'type': 'support', 'label': 1}, ignore_index=True)
...
support_scam_data = support_scam_data.map(format_cell)

# # Save the DataFrame to a CSV file
support_scam_data.to_csv('../data/support_scam_responses_20.csv', index=False) 