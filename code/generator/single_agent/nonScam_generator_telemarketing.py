...
def generate_llama_response():
    ...
    A phone conversation between a caller trying to do telemarketing on the receiver and a receiver who is a scam baiter who trying to proof whether the caller is a scammer. The receiver should realize that the caller isn't a scammer after few replies.
...
for i in range(num_responses):
    ...
    print(full_response)
    telemarketing_nonscam_data = telemarketing_nonscam_data._append({'dialogue': full_response, 'type': 'telemarketing', 'label': 0}, ignore_index=True)
...
telemarketing_nonscam_data.to_csv('telemarketing_nonscam_responses_150.csv', index=False)